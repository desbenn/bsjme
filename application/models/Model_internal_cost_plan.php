...
class Model_internal_cost_plan extends CI_Model
{
	...
	{
		if($id){
			$sql = "SELECT internal_cost_plan.*,billing_item.type as 'budget_type', billing_item.name as 'item_name', billing_item.cost as 'p_amount',(SELECT name FROM user where internal_cost_plan.updated_by=user.id) as 'updated'  
